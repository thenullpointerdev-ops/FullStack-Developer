// Adding fake data to our DB to test our endpoints if its successfuly buid it or not.
...
if (!connectionString) {
    throw new Error('DATABASE_URL is missing. Add it to your .env file before running the seed script.');
}
...
const userId = "0fbf6daf-e08d-4472-8ffb-5ba65058b679";
...
// when we create async function, we need to catch any errors
main().catch((err) => {
    console.error(err);
    // close server
    process.exit(1);
    // disconnect our table /DB
}).finally(async () => {
